...
const BatchList = () => {
  ...
  const getData = async ()=>{
    try {
        const res = await fetch("/getallsession");
...
const cStatus = async(id,st)=>{
  ...
  try {
     let res = await fetch(`/changesession/${id}`,settings);
...
const dellet = async(id)=>{
  try{
    const res = await fetch(`/deletesession/${id}`);
    ...
    <div className={style.main}>
        <div className={style.con}>
          <div className={style.head}>
                <h1><span> <MdOutlineDateRange/> </span>Session List</h1>
            </div>
                    ...
                    <tbody>
                        {
                            ...
                            <td className={style.cr}>
                                ...
                                <button onClick={()=>navigat(`/session/${e._id}`)}><BsClipboardData/> Details</button>
                            </td>
                        </tr>
                                )
                            })
